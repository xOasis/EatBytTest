...
public class Tab extends RecyclerView {
    ...
    public static final int[] rtl_RecyclerTabLayout = { 0x7f03013d, 0x7f03013e, 0x7f03013f, 0x7f030140, 0x7f030141, 0x7f030142, 0x7f030143, 0x7f030144, 0x7f030145, 0x7f030146, 0x7f030147, 0x7f030148, 0x7f030149, 0x7f03014a };
    ...
    private void getAttributes(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TabStyleableLayout,defStyle,R.style.rtl_RecyclerTabLayout);


        setIndicatorColor(a.getColor(R.styleable.TabStyleableLayout_rtl_tabIndicatorColor,0));



        setIndicatorHeight(a.getDimensionPixelSize(R.styleable
                .TabStyleableLayout_rtl_tabIndicatorHeight, 0));

        mTabTextAppearance = a.getResourceId(R.styleable.TabStyleableLayout_rtl_tabTextAppearance,
                R.style.selectedTabAppearance);

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.TabStyleableLayout_rtl_tabPaddingStart, 0);
        mTabPaddingStart = a.getDimensionPixelSize(
                R.styleable.TabStyleableLayout_rtl_tabPaddingStart,mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(
                R.styleable.TabStyleableLayout_rtl_tabPaddingTop,mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(
                R.styleable.TabStyleableLayout_rtl_tabPaddingEnd, mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(
                R.styleable.TabStyleableLayout_rtl_tabPaddingBottom, mTabPaddingBottom);

        if (a.hasValue(R.styleable.rtl_RecyclerTabLayout_rtl_tabSelectedTextColor)) {
            mTabSelectedTextColor = a
                    .getColor(R.styleable.TabStyleableLayout_rtl_tabSelectedTextColor, 0);
            mTabSelectedTextColorSet = true;
        }

        mTabOnScreenLimit = a.getInteger(
                R.styleable.TabStyleableLayout_rtl_tabOnScreenLimit, 0);


        if (mTabOnScreenLimit == 0) {
            mTabMinWidth = a.getDimensionPixelSize(
                    R.styleable.TabStyleableLayout_rtl_tabMinWidth, 0);
            mTabMaxWidth = a.getDimensionPixelSize(
                    R.styleable.TabStyleableLayout_rtl_tabMaxWidth, 0);
        }

        mTabBackgroundResId = a
                .getResourceId(R.styleable.TabStyleableLayout_rtl_tabBackground, 0);
        mScrollEanbled = a.getBoolean(R.styleable.TabStyleableLayout_rtl_scrollEnabled, true);
        a.recycle();
    ...
    public void setUpWithAdapter(Tab.Adapter<?> adapter) {
        mAdapter = adapter;
    ...
    protected static class RecyclerOnScrollListener extends OnScrollListener {
        ...
        public RecyclerOnScrollListener(Tab Tab,
        ...
        protected void selectCenterTabForRightScroll() {
            ...
            int center = mTab.getWidth() / 2;
            for (int position = first; position <= last; position++) {
                View view = mLinearLayoutManager.findViewByPosition(position);
                if (view.getLeft() + view.getWidth() >= center) {
                    mTab.setCurrentItem(position, false);
                    break;
                }
            }
        }
    ...
    protected static class ViewPagerOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public ViewPagerOnPageChangeListener(Tab Tab) {
            mTab = Tab;
        }

        @Override
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            mTab.scrollToTab(position, positionOffset, false);
        }
        ...
        public void onPageSelected(int position) {
            if (mScrollState == ViewPager.SCROLL_STATE_IDLE) {
                if (mTab.mIndicatorPosition != position) {
                    mTab.scrollToTab(position);
                }
            }
        }
    }
    ...
    public static class DefaultAdapter
            extends Tab.Adapter<DefaultAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(DefaultAdapter.ViewHolder holder, int position) {
            ...
            holder.title.setTextSize(TypedValue.COMPLEX_UNIT_SP,16);                  //if text is not in center position,use default size
           if(getCurrentIndicatorPosition() == position){
               holder.title.setTextSize(TypedValue.COMPLEX_UNIT_SP,24);                 //if text is not center position,use bigger size
           }
